...
fn get_expr_name(expr: &Expr) -> String {
    match expr {
        ...
        Expr::AttrGet(ref key, _) => match key.inner() {
...
impl VarRegistry {
    ...
    fn list(&self) -> Vec<Var> {
        self.names
            .iter()
            .enumerate()
            .map(|(index, name)| Var::new(index + self.offset, name.clone()))
            .collect()
    }
...
impl DebugLocalInfo {
    fn new(name: String, spc: usize, epc: usize) -> Box<DebugLocalInfo> {
        Box::new(DebugLocalInfo { name, spc, epc })
    }
}
...
impl<'p> Compiler<'p> {
        ...
        match v {
            ...
            None => {
                ...
                if index > (OPCODE_MAXBx as usize) {
                    panic!(
                        "{}:{:?} to many constants",
                        self.proto.source, self.proto.lineinfo
                    )
                } else {
                    index
                }
    ...
    fn register_local_var(&mut self, name: String) -> usize {
        ...
        self.proto
            .debug_locals
            .push(DebugLocalInfo::new(name, self.code.pc(), 0));
        self.reg_top += 1;
        ret
    }
        ...
        if self.block.ref_upval {
            match self.block.parent {
                Some(ref parent) => {
                    ...
                    self.code
                        .add_ABC(OP_CLOSE, x as i32, 0, 0, self.block.endline());
                    Some(x)
                }
                None => None,
            }
    ...
    fn load_rk(&mut self, reg: &mut usize, expr: &ExprNode, cnst: Rc<Value>) -> i32 {
        ...
        if cindex < opMaxIndexRk {
            ...
            *reg += 1;
            self.code
                .add_ABx(OP_LOADK, ret as i32, cindex, start_line(expr));
            ret as i32
        }
    }
    ...
    fn compile_table_expr(&mut self, mut reg: usize, table: &ExprNode, expr_ctx: &ExprContext) {
        ...
        reg += 1;
        self.code
            .add_ABC(OP_NEWTABLE, tablereg as i32, 0, 0, start_line(table));
        ...
        if let Expr::Table(ref fields) = table.inner() {
            ...
            for (i, field) in fields.iter().enumerate() {
                ...
                match field.key {
                    ...
                    Some(ref key) => {
                        ...
                        let opcode = if let Expr::String(_) = key.inner() {
                            OP_SETTABLEKS
                        } else {
                            OP_SETTABLE
                        };
                ...
                if (array_count != 0 && (flush == 0 || islast)) || lastvarargs {
                    ...
                    let b = if islast && field.val.inner().is_vararg() {
                        0
                    } else {
                        num
                    };
                    ...
                    self.code
                        .add_ABC(OP_SETLIST, tablereg as i32, b as i32, c as i32, line);
                    if c == 0 {
            ...
            self.code
                .set_argc(tablepc, int2fb(fieldlen as i32 - array_count));
    ...
    fn compile_fncall_expr(
        ...
        let (name, argc) = match expr.inner() {
            ...
            Expr::MethodCall(ref method) => {
                ...
                let c = self.load_rk(
                    &mut reg,
                    expr,
                    Rc::new(Value::String(method.method.clone())),
                );
                self.code
                    .add_ABC(OP_SELF, funcreg as i32, b as i32, c, start_line(expr));
                reg = b + 1;
                let reg2 = funcreg + 2;
                if reg2 > reg {
                    reg = reg2
        ...
        self.proto
            .debug_calls
            .push(DebugCall::new(name, self.code.last_pc()));
    ...
    fn compile_binary_arith_expr(
        ...
        self.code
            .add_ABC(opcode, a as i32, b as i32, c as i32, line);
    }

    fn compile_binary_rel_expr_aux(
        &mut self,
        mut reg: usize,
        opr: BinaryOpr,
        lhs: &ExprNode,
    ...
    fn compile_binary_log_expr_aux(
        &mut self,
        mut reg: usize,
        expr: &ExprNode,
        expr_ctx: &ExprContext,
        ...
        match expr.inner() {
            ...
            &Expr::BinaryOp(BinaryOpr::And, ref lhs, ref rhs) => {
                ...
                self.compile_binary_log_expr_aux(
                    reg,
                    lhs,
                    expr_ctx,
                    nextcondlabel,
                    ...
                    || opr == &BinaryOpr::GE =>
            {
                ...
                self.compile_binary_rel_expr_aux(
                    reg,
                    *opr,
                    lhs,
                    rhs,
            ...
            _ => {
                if !hasnextcond && thenlabel == elselabel {
                    ...
                    if sreg == a {
                        self.code
                            .add_ABC(OP_TEST, a as i32, 0, 0 ^ flip, start_line(expr));
                    } else {
                        self.code.add_ABC(
                            OP_TESTSET,
                            sreg as i32,
    ...
    fn compile_binary_log_expr(
        ...
        match opr {
            BinaryOpr::And => {
                ...
                self.set_label_pc(nextcondlabel, lastpc);
                self.compile_binary_log_expr_aux(
                    reg, rhs, expr_ctx, endlabel, endlabel, false, &mut lb,
                );
            }
            BinaryOpr::Or => {
                self.compile_binary_log_expr_aux(
    ...
    fn compile_binaryop_expr(&mut self, mut reg: usize, expr: &ExprNode, expr_ctx: &ExprContext) {
        match expr.inner() {
            ...
            Expr::BinaryOp(BinaryOpr::Concat, ref lhs, ref rhs) => {
                ...
                loop {
                    match current.inner() {
                        Expr::BinaryOp(BinaryOpr::Concat, _sublhs, ref subrhs) => {
                            crange += 1;
                            current = subrhs;
                        }
                        _ => break,
            ...
            Expr::BinaryOp(opr, ref lhs, ref rhs)
                if opr == &BinaryOpr::And || opr == &BinaryOpr::Or =>
            {
    ...
    fn compile_unaryop_expr(&mut self, mut reg: usize, expr: &ExprNode, expr_ctx: &ExprContext) {
        let (opcode, operand) = match expr.inner() {
            Expr::UnaryOp(UnaryOpr::Not, ref subexpr) => match subexpr.inner() {
                &Expr::True => {
                    self.code.add_ABC(
                        OP_LOADBOOL,
                        expr_ctx.savereg(reg) as i32,
                        0,
                        0,
                        start_line(expr),
                    );
        ...
        self.code
            .add_ABC(opcode, a as i32, b as i32, 0, start_line(expr));
    }
    ...
    fn const_fold(&mut self, expr: &ExprNode) -> Option<Expr> {
        match expr.inner() {
            Expr::UnaryOp(UnaryOpr::Minus, ref subexpr) => match self.const_fold(subexpr) {
                None => {
                    if let Expr::Number(n) = subexpr.inner() {
                        Some(Expr::Number(-n))
                    } else {
                        None
                    }
    ...
    fn compile_expr(&mut self, mut reg: usize, expr: &ExprNode, expr_ctx: &ExprContext) -> usize {
        ...
        match expr.inner() {
            &Expr::True => self
                .code
                .add_ABC(OP_LOADBOOL, svreg, 1, 0, start_line(expr)),
            &Expr::False => self
                .code
                .add_ABC(OP_LOADBOOL, svreg, 0, 0, start_line(expr)),
            &Expr::Nil => self
                .code
                .add_ABC(OP_LOADNIL, svreg, svreg, 0, start_line(expr)),
            &Expr::Number(f) => {
                let num = self.const_index(Rc::new(Value::Number(f)));
                self.code
                    .add_ABx(OP_LOADK, svreg, num as i32, start_line(expr))
            }
            &Expr::String(ref s) => {
                let index = self.const_index(Rc::new(Value::String(s.clone())));
                self.code
                    .add_ABx(OP_LOADK, svreg, index as i32, start_line(expr))
            }
            &Expr::Dots => {
                ...
                self.code
                    .add_ABC(OP_VARARG, svreg, expr_ctx.opt + 2, 0, start_line(expr));
            ...
            &Expr::Ident(ref s) => {
                ...
                match identtype {
                    ExprScope::Global => {
                        let index = self.const_index(Rc::new(Value::String(s.clone())));
                        self.code
                            .add_ABx(OP_GETGLOBAL, svreg, index as i32, start_line(expr))
                    }
                    ExprScope::Upval => {
                        let index = self.upval.register_unique(s.clone());
                        self.code
                            .add_ABC(OP_GETUPVAL, svreg, index as i32, 0, start_line(expr));
                    }
                    ExprScope::Local => {
                        let index = self.find_local_var(s).unwrap() as i32;
                        self.code
                            .add_ABC(OP_MOVE, svreg, index, 0, start_line(expr));
                    }
                    _ => unreachable!(),
                }
            }
            &Expr::AttrGet(ref obj, ref key) => {
                ...
                self.code
                    .add_ABC(opcode, a, b as i32, c as i32, start_line(expr));
            }
            &Expr::Table(_) => {
            ...
            &Expr::FuncCall(_) | &Expr::MethodCall(_) => {
                sused = self.compile_fncall_expr(reg, expr, expr_ctx)
            }
            ...
            &Expr::Function(ref params, ref stmts) => {
                ...
                self.code
                    .add_ABx(OP_CLOSURE, svreg, protono as i32, start_line(expr));
                for upv in &upvals.names {
                    let (op, mut index) = match self.block.find_var_block(upv) {
                        ...
                        None => match self.upval.find(upv) {
        ...
        for (i, expr) in lhs.iter().enumerate() {
            ...
            match expr.inner() {
                ...
                &Expr::AttrGet(ref obj, ref key) => {
                    ...
                    let keyks = if let Expr::String(_) = key.inner() {
                        true
                    } else {
                        false
                    };
    ...
    fn compile_assign_stmt_right(
        ...
        lhs: &Vec<ExprNode>,
        rhs: &Vec<ExprNode>,
        mut acs: Vec<AssignContext>,
    ) -> (usize, Vec<AssignContext>) {
        ...
        while namesassigned < lennames {
            ...
            if namesassigned < rhs.len()
                && rhs[namesassigned].inner().is_vararg()
                && (lenexprs - namesassigned - 1) <= 0
            {
                let opt = lennames - namesassigned - 1;
                let regstart = reg;
                let incr =
                    self.compile_expr(reg, &rhs[namesassigned], &ExprContext::with_opt(opt as i32));
                reg += incr;
            ...
            if ac.expr_ctx.scope == ExprScope::Table {
                match expr.inner() {
                    ...
                    _ => {
                        ...
                        self.code
                            .propagate_KMV(regtop, &mut ac.valrk, &mut reg, incr, true);
                    }
                }
            } else {
                ac.nmove = incr != 0;
            ...
            match acs[i].expr_ctx.scope {
                ...
                ExprScope::Global => {
                    if let Expr::Ident(ref s) = expr.inner() {
                        let index = self.const_index(Rc::new(Value::String(s.clone())));
                        self.code
                            .add_ABx(OP_SETGLOBAL, reg, index as i32, start_line(expr));
                        reg -= 1;
                    } else {
                        unreachable!()
                    }
                ...
                ExprScope::Upval => {
                    if let Expr::Ident(ref s) = expr.inner() {
                        let index = self.upval.register_unique(s.clone());
                        self.code
                            .add_ABC(OP_SETUPVAL, reg, index as i32, 0, start_line(expr));
                        reg -= 1;
                    } else {
                        unreachable!()
                    }
    ...
    fn compile_reg_assignment(
        ...
        if lennames > namesassigned {
            ...
            self.code
                .add_ABC(OP_LOADNIL, reg as i32, (reg + left) as i32, 0, line);
            reg += left;
        }
    ...
    fn compile_local_assign_stmt(
        ...
        if names.len() == 1 && values.len() == 1 {
            if let Expr::Function(_params, _stmts) = values[0].inner() {
    ...
    fn compile_branch_condition(
        ...
        let (flip, jumplabel) = if hasnextcond {
            (1, thenlabel)
        } else {
            (0, elselabel)
        };
    ...
    fn compile_if_stmt(&mut self, ifelsethen: &IfThenElse, startline: u32, _endline: u32) {
        let thenlabel = self.new_label();
        let elselabel = self.new_label();
        let endlabel = self.new_label();
    ...
    fn compile_gfor_stmt(&mut self, gfor: &GenericFor, startline: u32, endline: u32) {
        ...
        self.code
            .add_ABC(OP_TFORLOOP, rgen as i32, 0, nnames as i32, startline);
    ...
    fn compile_return_stmt(&mut self, exprs: &Vec<ExprNode>, startline: u32, _endline: u32) {
        let lenexprs = exprs.len();
        let mut reg = self.reg_top();
        let a = reg;
        ...
        self.code
            .add_ABC(OP_RETURN, a as i32, count as i32, 0, startline);
    }
                ...
                if blk.ref_upval {
                    match blk.parent {
                        Some(ref parent) => {
                            self.code.add_ABC(
                                OP_CLOSE,
                                parent.locals.last_index() as i32,
                                0,
                                0,
                                startline,
                            );
    ...
    fn compile_stmt(&mut self, stmt: &StmtNode) {
        ...
        match stmt.inner() {
            ...
            &Stmt::LocalAssign(ref names, ref values) => {
                self.compile_local_assign_stmt(names, values, startline)
            }
            ...
            &Stmt::While(ref cond, ref stmts) => {
                self.compile_while_stmt(cond, stmts, startline, endline)
            }
            &Stmt::Repeat(ref cond, ref stmts) => {
                self.compile_repeat_stmt(cond, stmts, startline, endline)
            }
            ...
            &Stmt::MethodDef(ref methoddef) => {
                ...
                let kreg = self.load_rk(
                    &mut regtop,
                    &methoddef.body,
                    Rc::new(Value::String(methoddef.method.clone())),
                );
                self.compile_expr(
                    regtop,
                    &methoddef.body,
                    &ExprContext::new(ExprScope::Method, REG_UNDEFINED, 0),
                );
                self.code.add_ABC(
                    OP_SETTABLE,
                    treg as i32,
    ...
    fn patchcode(&mut self) {
        let mut maxreg = if self.proto.param_count > 1 {
            self.proto.param_count as i32
        } else {
            1
        };
        let mut moven = 0;
        ...
        while pc < lastpc {
            ...
            match curop {
                ...
                OP_SETGLOBAL | OP_SETUPVAL | OP_EQ | OP_LT | OP_LE | OP_TEST | OP_TAILCALL
                | OP_RETURN | OP_FORPREP | OP_FORLOOP | OP_TFORLOOP | OP_SETLIST | OP_CLOSE => {}
                OP_CALL => {
            ...
            if curop == OP_MOVE {
                ...
                if moven > 1 {
                    ...
                    let c = if moven - 1 < OPCODE_MAXC as usize {
                        (moven - 1) as i32
                    } else {
                        OPCODE_MAXC
                    };
    ...
    fn compile_func_expr(
        ...
        self.end_scope();
        let _codestore = Instructions::new();
        ...
        for clv in &self.proto.constants {
            let sv = if let Value::String(ref s) = **clv {
                s.clone()
            } else {
                String::new()
            };
...
pub fn compile(stmts: Vec<StmtNode>, name: String) -> Result<Box<FunctionProto>> {
    ...
    par.set_vargs(true);
    let lineinfo = if stmts.len() > 0 {
        (start_line(&stmts[0]), end_line(&stmts[stmts.len() - 1]))
    } else {
        (1, 1)
    };
